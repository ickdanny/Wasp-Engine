...
namespace wasp::game::systems {
	...
	private:
        ...
        using TwoFramePosition = utility::TwoFrame<Point2>;
    ...
    public:
        ...
        void insert(
            ...
            AABB twoFrameEncompassingHitbox{
                math::makeEncompassingAABB(trueHitbox, pastHitbox)
            };
    ...
    private:
        ...
        bool collides(const AABB& hitbox) const {
            return math::collides(hitbox, bounds);
        }
        ...
        static bool collides(const Element& left, const Element& right) {
            ...
            //the true hitboxes collide or the objects collided in the subframe.
            return 
                math::collides(
                ...
                && 
                (
                    math::collides(left.trueHitbox, right.trueHitbox) 
                    || 
        ...
        static bool subFrameCollides(const Element& left, const Element& right) {
            ...
            if (largestSpeedRatio > 2.0f) {
                ...
                float baseRatio{ 1.0f / (numChecks + 1) };
                for (int i{ 1 }; i <= numChecks; ++i) {
                    float currentRatio{ baseRatio * i };
                    ...
                    if (
                        math::collides(leftInterpolatedHitbox, rightInterpolatedHitbox)
                    ) {
                        return true;
                    }
                }
