...
namespace wasp::game::resources {
	...
	resource::ResourceBase* DialogueStorage::loadFromFile(
		...
		std::shared_ptr<ResourceType> resourceSharedPointer{
			std::make_shared<ResourceType>(
				id,
				fileOrigin,
				std::make_shared<Dialogue>(parseDialogueFile(fileOrigin.fileName))
			)
		};
	...
	resource::ResourceBase* DialogueStorage::loadFromManifest(
		...
		std::shared_ptr<ResourceType> resourceSharedPointer{
			std::make_shared<ResourceType>(
				id,
				manifestOrigin,
				std::make_shared<Dialogue>(parseDialogueFile(fileName))
			)
		};
