...
namespace wasp::game::gameresource {
	...
	resource::ResourceBase* MidiSequenceStorage::loadFromFile(
		...
		std::shared_ptr<ResourceType> resourceSharedPointer{
			std::make_shared<ResourceType>(
				...
				std::make_shared<MidiSequence>(
					sound::midi::parseMidiFile(fileOrigin.fileName)
				)
			)
		};
	...
	resource::ResourceBase* MidiSequenceStorage::loadFromManifest(
